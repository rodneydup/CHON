...
void updateVelocities2D(std::array<std::array<Particle, NY>, NX> &particle, double springLength,
                        bool freedom[3], float kH, float kV, float m, float b, int step) {
  ...
  for (int y = 0; y < NY; y++)
    for (int x = 0; x < NX; x++) {
      if (x < NX - 1 && y < NY - 1) {
        ...
        forcesX =
          calculateForcesFixed(particle[x][y], particle[x + 1][y], springLength, kH, freedom);
        forcesY =
          calculateForcesFixed(particle[x][y], particle[x][y + 1], springLength, kV, freedom);
        for (int j = 0; j < 3; j++) {
          particle[x][y].acceleration[j] +=
            forcesX[j] + forcesY[j];  // acceleration due to right and above particle spring
